...
def get_chat_history(session_id):
    messages = list(
        chat_history_collection.find({"session_id": session_id}).sort("timestamp")
    )
    chat_history = [
...
async def query(request: QueryRequest):
    ...
    if not docs:
        ...
        raise HTTPException(
            status_code=404, detail="Sorry, we couldn't find any relevant answer"
        )
    ...
    try:
        response = openai.chat.completions.create(
            ...
            + [
                {
                    "role": "system",
                    "content": "You are a helpful assistant for a dental company named Apollo. If you can't find the answer, tell the user to reach Apollo using information shared. The document is not provided by the user but the company Apollo but it added there. Don't mention it in our discussion. ",
                },
                {
                    "role": "user",
                    "content": f"Answer the question based on the following:\n\n{relevant_text}\n\nQuestion: {request.query}. If you don't find an answer, ask me to contact Apollo with their phone number. Do not use the word document to represent the data provided to you.",
                },
            ],
            max_tokens=150,
        )
